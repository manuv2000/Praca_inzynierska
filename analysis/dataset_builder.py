...
def _drop_edge_windows(df: pd.DataFrame) -> pd.DataFrame:
    """
    Obowiązkowy filtr na start:
    usuwa pierwsze i ostatnie okno per run_id (często są 'ucięte').
    """
    if df.empty:
        return df

    # zakładamy że mamy win_index, run_id
    g = df.groupby("run_id", sort=False)
    keep_mask = []

    for _, part in g:
        if len(part) < 3:
            # jeżeli run ma <3 okna, to nie ma czego "obcinać" sensownie -> odrzuć cały run
            keep_mask.extend([False] * len(part))
            continue

        # zachowaj wszystko poza min i max win_index
        wmin = part["win_index"].min()
        wmax = part["win_index"].max()
        keep = ~part["win_index"].isin([wmin, wmax])
        keep_mask.extend(list(keep.values))
...
def build_dataset(
    *,
    ...
    # sanity: typy
    if "win_index" in out.columns:
        out["win_index"] = pd.to_numeric(out["win_index"], errors="coerce").fillna(-1).astype(int)

    # filtr: drop first+last window per run
    if drop_edges:
        out = _drop_edge_windows(out)
...
def main() -> None:
    ...
    df, stats = build_dataset(
        ...
        drop_edges=not bool(args.no_drop_edges),
    )
